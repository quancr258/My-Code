...
/* skt: so khung trang
	...
	ct: quyet dinh chuong trinh co duoc tiep tuc thuc hien hay k
*/
//------------KHOI TAO DU LIEU------------------------
void khoitao(ptrang a[],int &n,int &skt,int &loi)
{
	...
	printf("so phan tu chuoi truy xuat = ");scanf("%d",&n);
	printf("so khung trang = ");scanf("%d",&skt); 
	loi = n;
	printf("Nhap Chuoi truy xuat:\n");
	for(i=1;i<=n;i++)
		scanf("%d",&a[i].ptu);
}
...
int main()
{
	...
	else
	{
		do
		{
			printf("\nYeu cau thay the trang (0 = FIFO; 1 = LRU; 2 = OPT): ");scanf("%d",&ycttt);
		}while(ycttt>2 || ycttt<0);
		khungtrang(b,skt);
		...
		printf("\nBan co muon thuc hien chuong trinh voi thuat toan khac khong? (0 = Co; 1 = Khong): ");scanf("%d",&ct);
	}
	}while(ct==0);
	getch();
}
